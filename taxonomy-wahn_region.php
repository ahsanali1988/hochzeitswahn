<?php
/**
 * The template for displaying the wahnbuch regio pages.
 *
...
$term_name = get_term_by( 'id', $wp_query->get_queried_object_id(), 'wahn_region' );
      ...
      <nav class="sub-category-nav">
        ...
        <ul class="sub-cat-nav-catlist unstyled">
          <?php             
            ...
            echo '<ul>';
            echo '<li class="tags_overview"> <a href="'.esc_url(remove_query_arg( 'wahn_categorie',  $current_url)).'" title="'.__('Alle Kategorien anzeigen', 'fwbase').'" rel="nofollow">'.__('Alle Kategorien anzeigen', 'fwbase').'</a> </li>';            
            foreach( $terms as $term ) {
              if( $term->slug === $cat_query ) {
                echo '<li class="active"> <a href="'.$current_url.'/?wahn_categorie='.$term->slug.'" title="'.$term->name.'" rel="nofollow">'.$term->name.'</a> </li>';
              } else {
                echo '<li> <a href="'.$current_url.'/?wahn_categorie='.$term->slug.'" title="'.$term->name.'" rel="nofollow">'.$term->name.'</a> </li>'; 
              }                
            }
            echo '</ul> </li>';            
          ?>          
        ...
        <ul class="sub-cat-nav-catlist unstyled">
          <?php             
            ...
            $add_query = ( get_query_var( 'wahn_categorie' ) != '' ) ? '?wahn_categorie='.get_query_var( 'wahn_categorie' ) : '';

            echo '<li class="wahn_region"> <span>'. __( 'Suche in deiner Region:', 'fwbase' ) .'</span>';
            echo '<ul>';                
            foreach( $terms as $term ) {
              if( $term_name->slug === $term->slug ) {   
                echo '<li class="active"> <a href="'.get_term_link( $term->slug, 'wahn_region' ).$add_query.'" title="'.$term->name.'" rel="nofollow">'.$term->name.'</a> </li>';
              } else {
                echo '<li> <a href="'.get_term_link( $term->slug, 'wahn_region' ).$add_query.'" title="'.$term->name.'" rel="nofollow">'.$term->name.'</a> </li>';
              }
            }
            echo '</ul> </li>';
          ?>
