...
?>
  ...
  <header class="wahnbuch-header">
    ...
    <div class="wahnbuch-header-info">
      ...
      <p> 
        <?php
          if( !empty($cat_query) ) {
            $termID = get_term_by('slug', $cat_query, 'wahn_categorie');
            $term_descr = get_field('wahn_cat_description', $termID);
          }
  ...
  <main id="main" class="site-main" role="main">
      ...
      <?php if( $cat_query != '' || $reg_query != '' ) : ?>
  	    ...
  	    <?php 	  
          if($cat_query != ''){echo '<a class="filter-to-delete" href="'.$current_url.'?wahn_categorie=&wahn_region='.$reg_query.'" title="'.__('Kategorie l&ouml;schen', 'fwbase').'">'.__('Kategorie l&oumlschen', 'fwbase').'</a>';}
          if($reg_query != ''){echo '<a class="filter-to-delete" href="'.$current_url.'?wahn_categorie='.$cat_query.'&wahn_region=" title="'.__('Region l&ouml;schen', 'fwbase').'">'.__('Region l&oumlschen', 'fwbase').'</a>';}
        ?>
  	  </div>
      <?php endif; ?>		      
          ...
          <?php if( empty($cat_query) && empty($reg_query) ) : ?>
            ...
            <?php $wahn_categories = get_terms('wahn_categorie', array('hide_empty' => false)); foreach($wahn_categories as $category) : ?>
            ...
            <li class="wahn-cat">  
              ...
              <article class="default-article-layout">
                ...
                <div class="entry-thumbnail">
                  ...
                  <a href="<?php echo $current_url.'?wahn_categorie='.$category->slug; ?>" title="<?php echo $category->name; ?>">
                  ...
                  <h5 class="entry-title"> <a href="<?php echo $current_url.'?wahn_categorie='.$category->slug; ?>" title="<?php echo $category->name; ?>"> <?php echo $category->name; ?> </a> </h5>
