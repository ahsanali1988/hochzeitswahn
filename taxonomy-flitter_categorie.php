<?php
/**
 * The template for displaying the flitterwochen cat pages.
 *
...
$term_name = get_term_by( 'id', $wp_query->get_queried_object_id(), 'flitter_categorie' );
      ...
      <nav class="sub-category-nav">
        ...
        <ul class="sub-cat-nav-catlist unstyled">
          <?php
            ...
            $add_query = ( get_query_var( 'flitter_region' ) != '' ) ? '?flitter_region='.get_query_var( 'flitter_region' ) : '';
            ...
            echo '<ul>';
              ...
              foreach( $terms as $term ) {
                if( $term_name->slug === $term->slug ) {
                  echo '<li class="active"> <a href="'.get_term_link( $term->slug, 'flitter_categorie' ).$add_query.'" title="'.$term->name.'" rel="nofollow">'.$term->name.'</a> </li>';
                } else {
                  echo '<li> <a href="'.get_term_link( $term->slug, 'flitter_categorie' ).$add_query.'" title="'.$term->name.'" rel="nofollow">'.$term->name.'</a> </li>';
                }
              }
            echo '</ul> </li>';
        ...
        <ul class="sub-cat-nav-catlist unstyled">
          <?php
            ...
            echo '<ul>';
              echo '<li class="tags_overview"> <a href="'.esc_url(remove_query_arg( 'flitter_region',  $current_url)).'" title="Alle Regionen" rel="nofollow">Alle Regionen</a> </li>';
              foreach( $terms as $term ) {
                if( $term->slug === $reg_query ) {
                  echo '<li class="active"> <a href="'.$current_url.'/?flitter_region='.$term->slug.'" title="'.$term->name.'" rel="nofollow">'.$term->name.'</a> </li>';
                } else {
                  echo '<li> <a href="'.$current_url.'/?flitter_region='.$term->slug.'" title="'.$term->name.'" rel="nofollow">'.$term->name.'</a> </li>';
                }
              }
            echo '</ul> </li>';
          ?>
