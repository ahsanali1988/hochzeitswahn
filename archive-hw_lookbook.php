<?php
/**
 * The template for displaying the lookbook archiv pages.
 *
      ...
      <nav class="sub-category-nav lookbook-filter">
        ...
        <a class="button submit" href="<?php echo esc_url(home_url(add_query_arg(array(),$wp->request))); ?>" title="<?php _e('Filter die Produkte', 'fwbase'); ?>"><?php _e('Filtern', 'fwbase'); ?></a>
  		  ...
  		  <?php 
    		  ...
    		  foreach( $look_filter as $delete_filter ) : 
    		  
    		    if( $delete_filter != '' ) : 
    		        		    
    		      if( $counter == count($look_filter) ) {
      		    
      		      $cleanURL = str_replace($delete_filter, '', $_SERVER["HTTP_HOST"].$_SERVER['REQUEST_URI']); 
